...
import { Flex, Box, Input, Button, Text, useColorModeValue } from '@chakra-ui/react'
...
export default function Chat() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const firebaseUserId = localStorage.getItem('userID')
      ...
      const response = await fetch('https://api.wallie.ai/wallie', {
        ...
        body: JSON.stringify({ prompt: inputCode, userid: firebaseUserId })
      })
...
const ChatBubble = ({ text, isUser }: ChatMessage) => {
  return (
    ...
    >
      ...
      <Text ml={isUser ? 0 : 2} mr={isUser ? 2 : 0}>
        {text}
      </Text>
      {!isUser && (
        <Box ml={2}>
